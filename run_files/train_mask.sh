...
docker run -it --rm --gpus "device=1" \
  ...
  --configs defaults,physics \
  --logdir /logdir/dreamerv3-objectdrop-image-only-{timestamp} \
  --task physics_informed \
  --env.physics.path /object_drop/ \
  --env.physics.pattern "*.png" \
  --env.physics.subdir_prefix "_run631" \
  --env.physics.max_subdirs 1 \
  --env.physics.file_prefix "rgb" \
  --env.physics.exts .png \
  --batch_size 16 \
  --batch_length 64 \
  --run.envs 1 \
  --run.steps 300000 \
